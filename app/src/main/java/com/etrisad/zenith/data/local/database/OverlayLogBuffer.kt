...
object OverlayLogBuffer {
    private const val MAX_ENTRIES = 500
    private val entries = CopyOnWriteArrayList<OverlayLogEntry>()

    fun d(tag: String, message: String) {
        add(OverlayLogEntry(level = OverlayLogEntry.Level.D, tag = tag, message = message))
    }

    fun w(tag: String, message: String) {
        add(OverlayLogEntry(level = OverlayLogEntry.Level.W, tag = tag, message = message))
    }

    fun e(tag: String, message: String) {
        add(OverlayLogEntry(level = OverlayLogEntry.Level.E, tag = tag, message = message))
    }

    private fun add(entry: OverlayLogEntry) {
        entries.add(entry)
    ...
    fun getAll(): List<OverlayLogEntry> = entries.toList()

    fun getFiltered(tag: String? = null, level: OverlayLogEntry.Level? = null): List<OverlayLogEntry> {
    ...
    fun copyAll(): String {
        ...
        sb.appendLine("=== Zenith Overlay Log Dump ===")
